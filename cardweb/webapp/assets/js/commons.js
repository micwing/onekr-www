$(function() {
	$(document).on("click", ".ajax-post", function(d) {
		d.preventDefault();
	...
	$(document).on(
			...
			function(f) {
				...
				var b = $(this);
				$("[type=submit]", b).addClass("disabled").attr("disabled",
						"true");
				...
				a(d, c, function(e) {
					handleAjax(e);
					$("[type=submit]", b).removeClass("disabled").removeAttr(
							"disabled")
				}, "json");
				return false
			})
...
function handleAjax(b) {
	...
	if (b.status == 1) {
		if (b.msg) {
			op_success(b.msg, "娓╅Θ鎻愮ず")
		}
		if (b.html) {
			$("#mes_con").prepend(b.html).hide().fadeIn("slow")
		}
	} else {
		op_error(b.msg, "娓╅Θ鎻愮ず")
	}
	var c = 1500;
	if (b.url == "refresh") {
		setTimeout(function() {
