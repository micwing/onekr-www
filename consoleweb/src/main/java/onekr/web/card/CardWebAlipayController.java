...
//			out.println("验证成功<br />");
			...
			if (user == null) {
				user = userBiz.getAnonymous();
			}
			cardMakeCodeBiz.generateNewCode(user.getName(), user.getId());
			mav.addObject("makecode", );
...
//			//该页面可做页面美工编辑
//			out.println("验证失败");
			throw new AppException(ErrorCode.SERVER_ERROR, "验证失败");
		}
