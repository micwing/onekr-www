...
public class ConsoleScriptController extends BaseController {
	...
	public ModelAndView executeScript(@PathVariable("scriptId") Long scriptId) {
		...
		if (script.getScriptType().ordinal() == ScriptType.ARTICLE.ordinal()) {
			mav.setViewName(CONSOLE+"script-article-action");
			...
			mav.setViewName(CONSOLE+"script-url-action");
		}
	...
	public ModelAndView scripts() {
		ModelAndView mav = new ModelAndView(CONSOLE+"scripts");
	...
	public ModelAndView createScript() {
		ModelAndView mav = new ModelAndView(CONSOLE+"script");
		return mav;
	}
